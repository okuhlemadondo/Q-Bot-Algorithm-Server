...
class dwx_client():
    ...
    def __init__(self, event_handler=None,
                 metatrader_dir_path='C:\\Users\\okuhlemadondo\\AppData\\Roaming\\MetaQuotes\\Terminal\\D0E8209F77C8CF37AD8BF550E51FF075\\MQL5\\Files',
        ...
        self.path_commands_prefix = join(metatrader_dir_path,
                                         'DWX', 'DWX_Commands_')
        self.path_ASKNN_prefix = join(metatrader_dir_path,
                                         'DWX', 'DWX_ASKNN_')
        self.path_BIDNN_prefix = join(metatrader_dir_path,
                                         'DWX', 'DWX_BIDNN_')
        self.path_ASK_prefix = join(metatrader_dir_path,
                                         'DWX', 'DWX_ASK_')
        self.path_BID_prefix = join(metatrader_dir_path,
                                         'DWX', 'DWX_BID_')
        self.path_tr_prefix = join(metatrader_dir_path,
                                         'DWX', 'DWX_TR_')
                ...
                for symbol in data.keys():
                    if symbol not in self._last_market_data or self.market_data[symbol] != self._last_market_data[symbol]:
                        self.event_handler.on_tick(symbol,
                                                   ...
                                                   self.market_data[symbol]['bf1'],
                                                   self.market_data[symbol]['bf2'],
                                                   self.market_data[symbol]['bf3'],
                                                   self.market_data[symbol]['bf4'],
                                                   self.market_data[symbol]['bf5'],
                                                   self.market_data[symbol]['ask'],
                                                   self.market_data[symbol]['af1'],
                                                   self.market_data[symbol]['af2'],
                                                   self.market_data[symbol]['af3'],
                                                   self.market_data[symbol]['af4'],
                                                   self.market_data[symbol]['af5']
                                                   )
    ...
    def subscribe_symbols_bar_data(self, symbols=[['Volatility 75 (1s) Index', 'M1']]):
    ...
    def ask(self, ask):
        ...
        self.send_ask('ASK', ask)

    def bid(self, bid):

        self.send_bid('BID', bid)
    
    def ask_nn_output(self, ask_out):

        self.send_asknn('ASK_NN_OUTPUT', ask_out)

    def bid_nn_output(self, bid_out):

        self.send_bidnn('BID_NN_OUTPUT', bid_out)

    def tr(self, num):

        self.send_should_trade('TR',num)
    ...
    def send_asknn(self, command, content):
        ...
        while now < end_time:
            ...
            for i in range(self.num_command_files):
                # only send commend if the file does not exists so that we do not overwrite all commands.
                file_path = f'{self.path_ASKNN_prefix}{i}.txt'
                if not exists(file_path):
                    try:
                        with open(file_path, 'w') as f:
                            f.write(f'<:{command}|{content}:>')
    ...
    def send_bidnn(self, command, content):
        ...
        while now < end_time:
            ...
            for i in range(self.num_command_files):
                # only send commend if the file does not exists so that we do not overwrite all commands.
                file_path = f'{self.path_BIDNN_prefix}{i}.txt'
                if not exists(file_path):
                    try:
                        with open(file_path, 'w') as f:
                            f.write(f'<:{command}|{content}:>')
    ...
    def send_ask(self, command, content):
        ...
        while now < end_time:
            ...
            for i in range(self.num_command_files):
                # only send commend if the file does not exists so that we do not overwrite all commands.
                file_path = f'{self.path_ASK_prefix}{i}.txt'
                if not exists(file_path):
                    try:
                        with open(file_path, 'w') as f:
                            f.write(f'<:{command}|{content}:>')
    ...
    def send_bid(self, command, content):
        ...
        while now < end_time:
            ...
            for i in range(self.num_command_files):
                # only send commend if the file does not exists so that we do not overwrite all commands.
                file_path = f'{self.path_BID_prefix}{i}.txt'
                if not exists(file_path):
                    try:
                        with open(file_path, 'w') as f:
                            f.write(f'<:{command}|{content}:>')
    ...
    def send_should_trade(self, command, content):
        ...
        while now < end_time:
            ...
            for i in range(self.num_command_files):
                # only send commend if the file does not exists so that we do not overwrite all commands.
                file_path = f'{self.path_tr_prefix}{i}.txt'
                if not exists(file_path):
                    try:
                        with open(file_path, 'w') as f:
                            f.write(f'<:{command}|{content}:>')
